...
public class TrendChart extends ATrendChart {
    ...
    public TrendChart(Context context, LottoTrendView lottoTrendView) {
...
//    }
//
//    public void setBlueCountCount(int count){
//        this.redCount=count;
//    }
...
//            }
            if ("00".equals(str)) {
                ...
                this.blueCount = 16;
                this.redCount = 33;
                drowBuleLine(arrayList);
            }else if("01".equals(str)){
                //修改
                this.blueCount = 0;
                this.redCount = 27;
                drowRedLine(arrayList);
            }
...
//            this.mPathPoint.reset();
//            this.mScaleRange = new float[]{0.0f, 2.0f};
//            if ("00".equals(this.mLotteryType)) {
                //修改此处控制绘制蓝色球  红色球
//                this.redCount = 33;
//                //修改
//                this.blueCount = 16;
...
//                }
//            }
             if ("50".equals(this.mLotteryType)) {
                this.redCount = 35;
                this.blueCount = 12;
            }
    ...
    private void drowRedLine(ArrayList<TrendData> arrayList){
        for (int i = 0; i < arrayList.size(); i++) {
            TrendData trendData = (TrendData) arrayList.get(i);
            if ("row".equals(trendData.getType())) {
                ...
                for (int i2 = 0; i2 < split.length; i2++) {
                    if (split[i2].equals("0")) {
                      //  float f = (((((float) this.redCount) + 0.5f) + ((float) i2)) * ((float) this.mXItemWidth)) + ((float) this.mDivWidth);
                        float f = ((((float) i2)) * ((float) this.mXItemWidth))+((float)(mXItemWidth/2));
                        float f2 = (((float) i) + 0.5f) * ((float) this.mXItemHeight);
                        if (i == 0) {
                            this.mPathPoint.moveTo(f, f2);
                        } else {
    ...
    private void drowBuleLine(ArrayList<TrendData> arrayList){
        for (int i = 0; i < arrayList.size(); i++) {
            TrendData trendData = (TrendData) arrayList.get(i);
            if ("row".equals(trendData.getType())) {
                String[] split = trendData.getBlue().split(",");
